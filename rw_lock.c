...
Compile:  gcc -g -Wall -o rw_lock rw_lock.c -pthread -lm

Run:./rw_lock <number of threads> <n> <m> <mMember> <mInsert> <mDelete> <sample_size>
...
//thread function
void* thread_oparation(void* thread_id);
...
void* thread_oparation(void* thread_id)
{
	...
	while (count < local_m)// as local_m is local for each thread, it is compared with the local variable 'count'
	{
		...
		if (random_select == 0 && member_completed == 0){
			...
			//acquire the lock here as we are accessing the global variable count_member and the function Member()
